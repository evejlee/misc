...
#ifdef DIAG
		std::cerr << "doHull:: " << v << " " << i << "," << i+1 << std::endl;
#endif
		v.normalize();
...
htmInterface::setPolyCorner(SpatialVector &v) {
    ...
    // don't insert if already existing
    for(i = 0; i < len; i++) {
        if (v == polyCorners_[i].c_) {
            return;
        }
    }
    ...
    if(len < 2) {
        ...
        // If there are zero or 1 points in the existing vector, just append.
        //
        // dcd: need two steps here: the resize adds one element to end of
        // polyCorners the old insert function also returned the new length so I
        // had to get that in the second step len = polyCorners_.size();

        polyCorners_.resize(len + 1);
        polyCorners_[len].c_ = v;
        ...
        if( (polyCorners_[0].c_ ^ polyCorners_[1].c_)*v > 0 ) {
            ...
            polyCorners_.resize(3);
            polyCorners_[2].c_ = v;

        } else if( (polyCorners_[0].c_ ^ polyCorners_[1].c_)*v < 0 ) {

            // Push element 1 to the end, and replace 1 with the new value
            polyCorners_.push_back(polyCorners_[1]);
            polyCorners_[1].c_ = v;
        ...
        // > 2 elements already exist in the vector
        //
...
#ifdef DIAG
                std::cerr << "setPolyCorner: Insert after " << i << " length = " << len << std::endl;
#endif
                if(i == len-1) {
                    // append if last: just push_back a copy of the first one
                    // and then copy in v
                    polyCorners_.push_back(polyCorners_[0]);
                    polyCorners_[len].c_ = v;

                } else {
                    // was: polyCorners_.insert(polyCorners_.end() - i - 1);
                    // which gives a compile error on newer g++ since a value is 
                    // required

                    // insert places a new element *before* the position
                    polyCorners_.insert(polyCorners_.end() - i - 1, polyCorners_[0]);
                    polyCorners_[i+1].c_ = v;
                }
                break;
            }
            // std::cerr << "setPolyCorner: still living" << std::endl;
        }
    }
#ifdef DIAG
    std::cerr << "QL: Polygon: now " << polyCorners_.size() << std::endl;
    for(i = 0; i < polyCorners_.size(); i++)
        std::cerr << polyCorners_[i].c_ << std::endl;
#endif
}
