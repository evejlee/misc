...
struct JSONValue {
        ...
        // use formattedWrite since format() doesn't do enums
        long getLong() {
            if (this.type != JSON_TYPE.INTEGER) {
                auto writer = appender!string();
                formattedWrite(writer, 
                        "this is a %s type not an INTEGER",this.type);
                throw new Exception(writer.data);
            }
            return this.integer;
        }
        real getReal() {
            if (this.type != JSON_TYPE.FLOAT) {
                auto writer = appender!string();
                formattedWrite(writer, 
                        "this is a %s type not an FLOAT",this.type);
                throw new Exception(writer.data);
            }
        ...
        string getString() {
            if (this.type != JSON_TYPE.STRING) {
                auto writer = appender!string();
                formattedWrite(writer, 
                        "this is a %s type not an STRING",this.type);
                throw new Exception(writer.data);
            }
            return this.str;
        }
        bool getBool() {
            bool bval;
            if (this.type == JSON_TYPE.TRUE) {
                bval = true;
            } else if (this.type == JSON_TYPE.FALSE) {
                bval = false;
            } else {
                auto writer = appender!string();
                formattedWrite(writer, "this is a %s type not a boolean",this.type);
                throw new Exception(writer.data);
            }
            return bval;
        }
        ...
        JSONValue[] getArray() {
            if (this.type != JSON_TYPE.ARRAY) {
                auto writer = appender!string();
                formattedWrite(writer, "this is a %s type not an ARRAY",this.type);
                throw new Exception(writer.data);
            }
            return this.array;
        }
        JSONValue[string] getDict() {
            if (this.type != JSON_TYPE.OBJECT) {
                auto writer = appender!string();
                formattedWrite(writer, "this is a %s type not an OBJECT",this.type);
                throw new Exception(writer.data);
            }
            return this.object;
        }

        // for number types we can do coercion to other numbers
        // and strings
        T coerce(T)()
        {
            Variant v;
            if (this.type == JSON_TYPE.INTEGER) {
                v = this.integer;
            } else if (this.type == JSON_TYPE.FLOAT) {
                v = this.floating;
            } else {
                auto writer = appender!string();
                formattedWrite(writer, 
                        "coerce requires INTEGER or FLOAT, this is a %s",this.type);
                throw new Exception(writer.data);
