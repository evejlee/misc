...
struct JSONValue {
        ...
        string toString() {
            string val;
            switch (this.type) {
                case JSON_TYPE.STRING:  val = this.str;         break;
                case JSON_TYPE.FLOAT: val = format("%.19g", this.floating); break;
                case JSON_TYPE.INTEGER: val = format("%s", this.integer); break;
                default: throw new Exception(format(
                   "Cannot convert json type '%s' to string", this.type));
                         break;
            }
            return val;
        }
        long toLong() {
            long val;
            switch (this.type) {
                //case JSON_TYPE.STRING:  val = this.str.to!long();  break;
                case JSON_TYPE.FLOAT: val = cast(long)this.floating; break;
                case JSON_TYPE.INTEGER: val = this.integer; break;
                default: throw new Exception(format(
                   "Cannot convert json type '%s' to long", this.type));
                         break;
            }
            return val;
        }
        real toReal() {
            real val;
            switch (this.type) {
                //case JSON_TYPE.STRING:  val = this.str.to!real();  break;
                case JSON_TYPE.FLOAT: val = this.floating; break;
                case JSON_TYPE.INTEGER: val = cast(real)this.integer; break;
                default: throw new Exception(format(
                   "Cannot convert json type '%s' to real", this.type));
                         break;
            }
            return val;
        }
