...
c--------------------------------------------------
c     This program :
c        Calculates weights for training set objects such
c        that the weighted training set distributions match
c        the photometric sample distributions.
c     INPUTS:
c        Reads in:
c           1) Training set filename
c           2) Number of letters in the filename
c           3) Photometric sample filename
c           4) Number of letters in the filename
c           5) variable: dotrain = 0 or 1
c             * If 0 -> will NOT find neighbors of training set, but simply
c               read their distances from a file.
c             * If 1 -> will go through the calculation for the training set.
c           6) variable: docolor = 0 or 1
c             * If 0 -> will compute/read distances using magnitudes.
c             * If 1 -> will compute/read distances using colors.           
c           7) variable: fixneigh = 0 or 1
c             * If 0 -> fixed volume. Will use volume defined by nneighbors
c               to find number of objects in photometric sample.
c             * If 1 -> fixed number of neighbors. Will use the same number
c               of neighbors both in the training set and photometric sample.
c           8) Number of nearest neighbors 
c     OUTPUTS:
c         (1)File with distances of training set objects to furtherest neighbor.
c            This can be used for future computations setting dotrain=0.
c         (2)File with zphot, zspec, weights, magnitudes for training set 
c            objects.
c--------------------------------------------------      
...
c      real*8 u(n,ncoefmax),v(ncoefmax,ncoefmax),w(ncoefmax)
      real*8 no
...
c Temporary holders for numerical command line arguments
...
ccccccccccccccccccccccccccccc
c Assigning file names to variables. Not sure why marcos did this.
ccccccccccccccccccccccccccccc
...
ccccccccccccccccccccccccccccc
c Assigning name to file where training set distances will be 
c written to/read from.
ccccccccccccccccccccccccccccc
      
      if (docolor.eq.0) then
c        namedist=outfilev(1:nletout)//".mag-train.dat"
        namedist=trainfile(1:nlettrain)//".mag-train.dat"
      else
c        namedist=outfilev(1:nletout)//".col-train.dat"
...
cccccccccccccccccccccccccccccccccccccccccccccccccccc
c Start computation/reading of neighbor distances for TRAINING SET
cccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccc      

      name=trainfilev(1:nlettrainv)
      call reading(name,nlettrainv,n,ntrain,zspec_t,zphot_t,
     *mag1_t,mag2_t,mag3_t,mag4_t,mag5_t)

c      write(*,*)zspec_t(1),zphot_t(1),
c     *mag1_t(1),mag2_t(1),mag3_t(1),mag4_t(1),mag5_t(1)
c      write(*,*)ntrain,nneighbor

c----------------------------
c Check if calculating or just reading from file
c----------------------------
...
ccccccc
c if calculating 
c open file where distances will be written
c record number of neighbors being used
ccccccc       
        open(1,file=namedist)
c        open(1,file='dist_train.dat')
...
c sorting them
cccccc
          call sort6(nneighbor,distance_n,
     *mag1_n,mag2_n,mag3_n,mag4_n,mag5_n,
     *wksp,iwksp)

ccccccc
c computing distance and density and writing file
ccccccc
      ...
      ELSE
c--------------------------
c If no computation, just read from file
c--------------------------
        write(*,'(A,$)')'Will just read file for training'
        write(*,*)

c        open(2,file='dist_train.dat')
        open(2,file=namedist)
        read(2,*)nneighborfile,docolorfile
cccccc
c Check that the number of neighbors used in the distance of training
c    set objects is the same you are using...
c
c Check that the distance of training set objects is calculated in
c    the same space (magnitude/color) you are using...
cccccc
...
c        write(*,*)'check no problems distance', 
c     *dist_max_t(1),dist_max_t(ntrain)
c        write(*,*)'check no problems density', 
c     *dens_t(1),dens_t(ntrain)
...
ccccccccccccccccccccccccccccccccccccccccccccccc
c     Start computation for PHOTOMETRIC SAMPLE
ccccccccccccccccccccccccccccccccccccccccccccccc
ccccccccccccccccccccccccccccccccccccccccccccccc      
      ...
      call reading(name,nletphotv,n,nphot,zspec_p,zphot_p,
     *mag1_p,mag2_p,mag3_p,mag4_p,mag5_p)


c      write(*,*)zspec_p(1),zphot_p(1),
c     *mag1_p(1),mag2_p(1),mag3_p(1),mag4_p(1),mag5_p(1)
c      write(*,*)nphot,nneighbor
...
c------------------------------
c calculating distance to photo object j
c------------------------------
cccccc
c Initializing
cccccc
...
cccccc
c Check if fixed number of neighbors or fixed volume
cccccc
        IF (fixvol.eq.0) THEN
cccccc
c finding neighbors
...
ccccccc
c computing distance and density
ccccccc
        ...
        ELSE
ccccccc
c just finding number in fixed volume/distance determined by training neighbors
ccccccc
...
c      write(*,*)'frango1',dist_max_p(1)
c      write(*,*)'frango2',distance(1),distance(2)
c      write(*,*)'frango3',numberreal(1)

c      write(*,*)'check no problems number', 
c     *numberreal(1),numberreal(ntrain)
c      write(*,*)'check no problems density', 
c     *dens_p(1),dens_p(ntrain)  



c---------
c    scaling back magnitudes to normal values and 
c    creating table file with zphot, zspec, weight and magnitudes
c---------

      call scalebackall(n,nphot,
     *mag1_p,mag2_p,mag3_p,mag4_p,mag5_p)
 
      call etable(outfile,
     *n,ntrain,
     *zphot_t,zspec_t,weight,
     *mag1_t,mag2_t,mag3_t,mag4_t,mag5_t)
...
c Get the length of a string, where blanks at the end are not
c counted as part of the string.  C is petter with the nulls.
...
c     Reading in file
c----------------------------------------------------------------------
      subroutine reading(name,nlet,n,ntrain,zspec2,zphot2,
     *mag1,mag2,mag3,mag4,mag5)
        ...
        open(1,file=name)
        do i=1,n
            read(1,*,end=3)zspec2(i),zphot2(i),
     *mag1(i),mag2(i),mag3(i),mag4(i),mag5(i)
...
ccccc
c scaling to work with  smaller magnitude numbers
c see subroutine "scale" bellow to see how magnitudes are scaled.
ccccc
...
c      gout=2./(magmax-magmin)*(gin-(magmax+magmin)/2.) !-1 at magmin 1 at magmax
...
c      gout=gin*(magmax-magmin)/2. +(magmax+magmin)/2.  ! -1 and 1
     
      return
      end

c----------------------------------------------------------------------
c     Another possible scaling
c----------------------------------------------------------------------
...
c---------------------------------------------------------------
c     Subroutine to find "m" nearest neighbors from vector arr(n)
c     Returns the index vector index(m) with the neighbors index.
c     Neighbors are not sorted tough, but you know who they are.
...
c----------------------------------------------------
c     this converts and integer to a character
c----------------------------------------------------
...
c----------------------------------------------------
c      this returns the factorial of a number
c----------------------------------------------------
      function fact(n)
...
c---------------------------------------------------------------------
c     find number of digits of a number
c---------------------------------------------------------------------
...
c---------------------------------------------------------------------
c     find number of digits of a string
c---------------------------------------------------------------------
...
c----------------------------------------------------------------------
c     This writes tablefile with zphot, zspec, weights and magnitudes.
c----------------------------------------------------------------------
      subroutine etable(outfile,
     *n,ntotalt,
     *zphot2,zspec2,weight,
     *mag1,mag2,mag3,mag4,mag5)
        ...
        open(3,file=outfile)
        do i=1,ntotalt
            write(3,66)zphot2(i),zspec2(i),weight(i),
     *mag1(i),mag2(i),mag3(i),mag4(i),mag5(i)
        end do
        close(3)

c 66     FORMAT(2f15.8,e14.6,100f15.8)
 66     FORMAT(100e14.6)
      end
...
c     Scaleback all
c----------------------------------------------------------------------
      subroutine scalebackall(n,ntotalp,
     *mag1,mag2,mag3,mag4,mag5)
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "arr(n)"
c     From Numerical Recipes
c-----------------------------------------------
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     From Numerical Recipes
c-------------------------------------------------
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     Simultaneously update vector "r2(n)"
c     From Numerical Recipes
c-------------------------------------------------
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     Simultaneously update vector "r2,r3"
c     From Numerical Recipes
c-------------------------------------------------
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     Simultaneously update vector "r2,r3,..r6"
c     From Numerical Recipes
c-------------------------------------------------
      SUBROUTINE sort6(n,r1,r2,r3,r4,r5,r6,
     *wksp,iwksp)
      INTEGER n,iwksp(n)
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     Simultaneously update vector "r2,r3,..r11"
c     From Numerical Recipes
c-------------------------------------------------
      SUBROUTINE sort11(n,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,
     *wksp,iwksp)
      INTEGER n,iwksp(n)
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     Simultaneously update vector "r2,r3,..r12"
c     From Numerical Recipes
c-------------------------------------------------
      SUBROUTINE sort12(n,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,
     *wksp,iwksp)
      INTEGER n,iwksp(n)
      REAL*8 r1(n),r2(n),r3(n),r4(n),r5(n),r6(n),r7(n),r8(n),r9(n)
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     Simultaneously update vector "r2,r3,..r13"
c     From Numerical Recipes
c-------------------------------------------------
      SUBROUTINE sort13(n,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,
     *wksp,iwksp)
      INTEGER n,iwksp(n)
      REAL*8 r1(n),r2(n),r3(n),r4(n),r5(n),r6(n),r7(n),r8(n),r9(n)
...
c-------------------------------------------------
c     Subroutine to sort "n" objects in vector "r1(n)"
c     Simultaneously update vector "r2,r3,..r14"
c     From Numerical Recipes
c-------------------------------------------------
      SUBROUTINE sort14(n,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,
     *r14,wksp,iwksp)
      INTEGER n,iwksp(n)
      REAL*8 r1(n),r2(n),r3(n),r4(n),r5(n),r6(n),r7(n),r8(n),r9(n)
...
c----------------------------------------
c     Used by sort subroutines above
c     From Numerical Recipes
c----------------------------------------
