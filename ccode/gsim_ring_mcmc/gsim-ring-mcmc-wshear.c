...
void print_one(const struct gmix_mcmc *self)
{
    ...
    size_t npars = MCA_STATS_NPARS(stats);

    long i1=npars-2;
    long i2=npars-1;

    printf("%ld %.16g %.16g %.16g %.16g %.16g %.16g %.16g %.16g %.16g %.16g\n",
           self->nuse,
           self->g[0],
           self->g[1],
           self->gsens[0],
           self->gsens[1],
           MCA_STATS_MEAN(stats,i1),
           MCA_STATS_MEAN(stats,i2),
           MCA_STATS_COV(stats,i1,i1),
           MCA_STATS_COV(stats,i1,i2),
           MCA_STATS_COV(stats,i2,i1),
           MCA_STATS_COV(stats,i2,i2));
