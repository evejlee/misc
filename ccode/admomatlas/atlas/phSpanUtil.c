...
#ifndef MAXSHORT
#define MAXSHORT ~((short)(1 << (8*sizeof(short) - 1)))
#endif
 ...
 */
void
phObjmaskRealloc(OBJMASK *sv,		/* Old Spanvec */
		  ...
		  )
{
    ...
    int x1, x2;				/* unpacked from spans[i].x[12] */
    int xmin, xmax;
    int nspans;
    SPAN *s1;
...
phObjmaskIntersect(
		   const OBJMASK *sv1, 
		   const OBJMASK *sv2,
		   int dy,		/* offset to be applied to sv2 */
		   int dx
)
{
   int irow1, irow2;
...
void
phObjmaskMerge(
	       OBJMASK *sv1,		/* target OBJMASK */
	       const OBJMASK *sv2,	/* input OBJMASK */
	       int dy,			/* Shift sv2 by this amount before
					   merging */
	       int dx
	       )
{
    ...
    int nspan;
    const int nspan1 = sv1->nspan;	/* unalias them */
    const int nspan2 = sv2->nspan;
		    ...
		    )
{
   ...
   for(i = 0; i < size; i++) {
      ...
      if(x2 < sv->cmin || x1 > sv->cmax  || y2 < sv->rmin || y1 > sv->rmax) {
	 continue;			/* no match on bounding box */
      }
      
      s = sv->s;
      for(j = 0; j < sv->nspan; j++) {
	 if(y2 < s[j].y) {		/* can't intersect, as s is sorted */
	    break;
	 }
	 if(y1 > s[j].y || x1 > s[j].x2 || x2 < s[j].x1) {
	    continue;
	 }
	 return 1;
      }
...
/*****************************************************************************/
/*
 * This function does the real work for phMaskIntersection() and
 ...
 */
	 ...
	 for(icol1 = irow1, icol2 = irow2;
	    ...
	    while(icol2 < sv2->nspan && s2[icol2].y == s2[irow2].y
		  && s2[icol2].x1 + col0_2 <= s1[icol1].x2 + col0_1) {
 ...
 * (see also phObjmaskNotIntersectionObjmask()). The mask sv1 is returned.
 *
 ...
 * (See also phObjmaskAndNotObjmask())
 *
 * This is currently an order sizeof(sv2) operation.
 * We may need to be more clever about this.
 */
